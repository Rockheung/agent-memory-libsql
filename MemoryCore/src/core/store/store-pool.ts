...
export type StoreMode = "sqlite" | "libsql" | "tcvdb" | "mongodb";
...
export class StorePool {
  ...
  async getStore(
    ...
    // (fork) libsql 은 프로세스 모드로만 정해진다 — 인스턴스별로 전달되는
    // config 가 없으므로 mongodb/tcvdb 의 존재 검사 뒤에 온다.
    const backend: StoreMode = mongoConfig
      ? "mongodb"
      : this.mode === "tcvdb" && vdbConfig ? "tcvdb"
      : this.mode === "libsql" ? "libsql"
      : "sqlite";
    const fingerprint =
    ...
    const pooledStore =
      ...
      : backend === "libsql" ? await this.createLibsqlStore(instanceId)
    ...
    const storeDesc =
      ...
      : backend === "libsql" ? `libsql @ ${this.libsqlCfg?.url}`
