...
- [x] **재부팅 검증 완료** (2026-08-29) — 두 번 재부팅. 개입 없이 30초 내 전부
      복구. 1차에서 cliproxy 가 wg0 보다 먼저 떠 exit 0 으로 죽던 것을 발견해
      `After=wg-quick@wg0` + `Restart=always` 드롭인으로 고쳤다.
- [x] **어댑터 state 파일 TTL** (2026-09-11 완료) — `memory_hook.py` 에 `sweep_state()`
      추가. `state_ttl_days` 기본 7일, `.last-sweep` 스탬프로 하루 1회, 이벤트 처리
      뒤에 돈다. `adapter.log` 는 보존. 실측으로 확인(12파일 → 오래된 6개만 삭제,
      재실행 무동작, TTL=0 비활성).
...
- [ ] **`spike/` 694줄** — 착수 전 가정 검증용 일회성 스크립트. 결과는
      ...
      둘지 지울지 결정만 하면 된다.
      **2026-09-11 추가 근거:** 마지막 실사용처였던 `spike/t/s3-backend.mjs`(274줄
      사본)가 없어졌다 — `__s3-backend.itest.ts` 를 실제 모듈
      `core/storage/s3-backend.ts` 로 돌렸다. 이제 `spike/` 를 참조하는 코드가 없다.
...
| 날짜 | 내용 |
|---|---|
| 2026-09-11 | **첫 동기화.** 포크(97f9465, 2026-08-15) 이래 처음. 17커밋, 릴리스 드롭 2개(v2.0.1 · v2.0.2-beta.1), 468파일 +52,974/-6,922. 브랜치 `try/upstream-merge-2026-09`. 충돌 7파일 16곳, 조용한 파손 4건. 검증 154/154 (계약 116 · 메모리 7 · 스킬 10 · 스킬HTTP 5 · S3 16). `rock/main` 미머지 |

절차와 함정은 [`../FORK.md`](../FORK.md) 의 "업스트림 동기화 절차" 참조.
**rebase 가 아니라 merge 다** — 초판의 rebase 절차는 폐기했다.
